pipeline {
    agent any
    environment {
	    SERVICE_REPO_NAME = 'RTM_GatewayService.git'
	    DOCKER_IMAGE = 'hyundooboo/rtm-gateway-service'
        ...
        stage('Update Helm values.yaml') {
            steps {
	            ...
	            withCredentials([usernamePassword(credentialsId: 'github-token', usernameVariable: 'GITHUB_USERNAME', passwordVariable: 'GITHUB_TOKEN')]) {
	                sh "git clone --version onprem https://${GITHUB_USERNAME}:${GITHUB_TOKEN}@github.com/Ready-to-Marry/RTM_infra.git rtm-helm"
